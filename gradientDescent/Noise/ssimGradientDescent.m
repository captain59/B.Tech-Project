function [reconstructed, original, mseMat, ssimMat] = ssimGradientDescent(A, varargin)
[~, ~, channels] = size(A);
if channels>1
    A = rgb2gray(A);
end
...
ssimMat = ssim(A, X);
for i=1:itterations
    derivative = learningParameter*ssimDerivativeGaussian(Y, X);
    X = X + derivative;
    ssimVal = ssim(A, X);
...
original = Y;
imwrite(uint8(reconstructed), 'SSIM_Reconstruction_Gaussian.jpg');
end